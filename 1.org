...
#+begin_src sql :engine mysql  :dbuser org-mode :dbpassword org-mode :database computerserviceooo :exports results :eval no-export :colnames no :hlines yes
...
#+ATTR_LATEX: :environment longtable :align |p{0.3cm}|p{1.5cm}|p{1.5cm}|p{2cm}|p{2cm}|p{0.3cm}|p{1cm}|p{2.2cm}|p{1cm}|p{0.3cm}|
#+RESULTS:
...
#+INCLUDE: ./employee.csv src csv

#+begin_src sql :engine mysql  :dbuser org-mode :dbpassword org-mode :database computerserviceooo :exports results :hlines yes
  SELECT * FROM country;
#+end_src
